...
export default function RecordDetailsModal({ title, subtitle, row, onClose }: RecordDetailsModalProps) {
                ...
                <div className="space-y-2">
                  {Object.entries(value as Record<string, unknown>).map(([subKey, subVal]) => (
                    <div key={subKey} className="flex items-start justify-between gap-4">
                      ...
                      <span className="text-right text-xs text-slate-700 break-all">
                        {formatValue(subVal)}
                      </span>
                    </div>
                  ))}
