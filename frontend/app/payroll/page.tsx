    ...
    () => [
      {
        ...
        detail: "Estimated gross (active employees)",
        icon: (
  ...
  return (
    <div className="page-shell">
        ...
        <div className="mb-4 grid gap-4 sm:grid-cols-2 xl:grid-cols-[repeat(auto-fit,minmax(220px,1fr))]">
          {stats.map((stat) => (
            <div key={stat.label} className={`stat-card ${stat.accentClass} group min-h-[128px] min-w-0`}>
              <div className="flex min-w-0 items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  <p className="break-words text-[11px] font-bold uppercase tracking-[0.16em] leading-snug text-slate-500">
                    {stat.label}
                  </p>
                  <p className="mt-2 break-words text-3xl font-black leading-tight text-slate-900">
                    {stat.value}
                  </p>
